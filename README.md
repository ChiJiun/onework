...
Docker Desktop 必須運作，Testcontainers 才能執行真實 PostgreSQL 併發測試；若 Docker 未啟動，Testcontainers 測試會自動略過。
...
| 自動化測試 | Service/API integration tests 與 PostgreSQL Testcontainers test，共 17 項 |
...
| Spring Security / JWT | BCrypt 密碼、JWT filter、stateless session、角色授權、管理員 IP 白名單登入與 401/403 JSON |
...
管理員也可以使用第二種登入模式：`POST /api/auth/admin/ip-login`。此模式仍需輸入管理員帳密，但伺服器會檢查來源 IP 是否在白名單內；白名單由 `ADMIN_LOGIN_ALLOWED_IPS` 設定，預設允許本機 `127.0.0.1` 與 `::1`。非 ADMIN 帳號或非白名單 IP 會回傳 HTTP `403`。
